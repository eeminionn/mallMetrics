...
# ============================================================
# PARÁMETROS DE LIMPIEZA ESTADÍSTICA
# ============================================================
# La idea central es: YOLO puede detectar mucho, pero la estadística solo usa
# personas validadas. Esto reduce falsos positivos y evita contar dos veces
# a la misma persona cuando el tracker cambia el ID.

# El modelo puede venir con conf=0.25 desde config.py, pero para analítica
# conviene ser más conservador.
tracking_confidence = max(float(confidence), 0.40)

# Una persona debe aparecer varios frames antes de entrar a las estadísticas.
min_confirmed_frames = 8

# Filtros de caja para descartar detecciones raras o demasiado pequeñas.
min_person_height_px = 45
min_person_area_px = 1200
min_width_height_ratio = 0.12
max_width_height_ratio = 1.25

# Reunificación simple de IDs: si aparece un track nuevo muy cerca de donde
# desapareció una persona hace poco, se considera la misma persona.
...
class RetailAnalyticsApp(BaseApp):
    ...
    def show_analysis_screen(self, zones):
        ...
        self.analysis_people_label = ctk.CTkLabel(side_panel, text="Personas validadas: 0", font=("Segoe UI", 14), text_color=TEXT_MAIN)
    ...
    def analysis_worker(self):
        try:
            ...
            # Filtros dinámicos según resolución del video.
            ...
            def register_zone_enter(person_id, raw_track_id, zone_id, current_time, frame_index, center_x, center_y):
                ...
                if zone["type"] == "frente_tienda":
                    ...
                    stair_track_state[(person_id, zone_id)] = {
                        "start_y": center_y,
            ...
            self.analysis_queue.put({"type": "status", "text": "Analizando video con filtros estadísticos..."})
            ...
            while True:
                ...
                if results[0].boxes.id is not None:
                    ...
                    for box, raw_track_id in zip(boxes, raw_track_ids):
                        ...
                        if not is_confirmed:
                            # Se dibuja como candidato, pero NO entra a las métricas.
                        ...
                        # Zonas con umbral de permanencia mínima.
                ...
                # Procesar personas confirmadas que desaparecieron dentro de una puerta.
                for person_id, last_data in list(last_seen_data.items()):
                    if person_id in visible_confirmed_people:
                        continue
                    if person_id in processed_inactive_people:
                        continue
            ...
            # Procesar tracks confirmados que quedaron sin cerrar al final del video.
            for person_id, last_data in list(last_seen_data.items()):
            ...
            self.analysis_queue.put({
                ...
                "text": (
                    ...
                    f"Personas validadas: {len(person_labels)}\n"
                    f"Detecciones rechazadas: {rejected_detection_count}"
                ),
            })
            ...
            while True:
                ...
                if item_type == "status":
                    ...
                    if self.analysis_people_label is not None:
                        self.analysis_people_label.configure(text=f"Personas validadas: {item.get('unique_people', 0)}")
                    ...
                    print(f"Personas validadas: {item.get('unique_people', 0)}")
